...
export default function GiftCategoriesPage() {
  ...
  const fetchCategories = async () => {
    setLoading(true);
    try {
      const response = await fetch(`${API_BASE_URL}/api/gift-categories?type=store`);
      if (response.ok) {
    ...
    <div className="flex flex-col space-y-8 max-w-4xl mx-auto pb-12">
      <div>
        ...
        <p className="text-[var(--text-secondary)] text-sm">
          Manage dynamic categories for the gift store.
        </p>
      </div>
          ...
          <div className="bg-[var(--card-bg)] rounded-2xl p-6 border border-gray-800 shadow-xl relative overflow-hidden backdrop-blur-xl">
            ...
            <form onSubmit={handleAddCategory} className="relative z-10 space-y-4">
              ...
              <button 
                ...
                className="w-full py-3 bg-gradient-to-r from-purple-600 to-pink-600 text-white rounded-xl text-sm font-semibold hover:shadow-lg hover:shadow-purple-500/25 transition-all disabled:opacity-50"
              >
