...
export default function ResellerApplicationsPage() {
  const mockApps = [
    { id: 'RAPP-101', applicant: 'Tom Hardy', uuid: 'U-77721', date: '2024-03-15', status: 'Pending' },
    { id: 'RAPP-102', applicant: 'Chris Evans', uuid: 'U-33311', date: '2024-03-14', status: 'Under Review' },
  ];

  return (
    <div className="flex flex-col space-y-6 max-w-7xl mx-auto pb-12">
      <div className="flex flex-col mb-4">
        <h1 className="text-3xl font-extrabold text-white mb-2">Reseller Applications</h1>
        <p className="text-[var(--text-secondary)] text-sm">
          Review and approve requests from users who want to become official coin resellers.
        </p>
      </div>
      ...
      <div className="bg-[var(--card-bg)] rounded-xl border border-gray-800 overflow-hidden shadow-lg">
        <div className="p-4 border-b border-gray-800 bg-[#18181b] flex justify-between items-center">
           <div className="flex gap-2">
             <button className="px-3 py-1.5 bg-amber-600 text-white rounded text-sm font-semibold">Pending (2)</button>
            ...
            <tbody className="divide-y divide-gray-800">
              {mockApps.map((row) => (
                <tr key={row.id} className="hover:bg-[#1f1f23] transition-colors">
                  <td className="px-6 py-4 text-amber-400 font-mono font-bold">{row.id}</td>
